...
export default function Catalog() {
  ...
  const searchItems = async () => {
    try {
      ...
      const searchParams = query ? { [searchType]: query } : {};
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-200 to-blue-500 pt-20 pb-10">
      ...
      <div id="results" className="w-full max-w-4xl mt-5">
        {items.length > 0 ? (
          ...
          <p className="text-center text-xl font-bold text-gray-600">{itemType === 'books' ? 'No books found' : 'No movies found'}</p>
        )}
      </div>
    </div>
  );
